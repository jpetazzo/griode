...
fn load_instruments() -> ServerState {
    ...
    let instrument_names: Vec<&str> = list_d
        .as_str()
        .lines()
        .filter(
            |x| {
                x
                    ...
                    .unwrap()
                    != b'#'
            }, // If it is not '#' then line accepted
        )
        .collect();
    ...
    for entry in dir.read_dir().expect("read_dir call failed") {
        if let Ok(entry) = entry {
            ...
            if p.as_path().is_file() && p.as_path().extension().is_none() {
                ...
                let instrument_name = p.file_name().unwrap().to_str().unwrap().to_string();
                ...
                if instrument_names.contains(&instrument_name.as_ref()) {
                    instruments.insert(instrument_name.clone(), p.to_str().unwrap().to_string());
                } else {
                }
            }
        }
